---
type: yoga-asana
title: "シータリー（śītalī）"
description: "HYP第2章収録（プラーナーヤーマ（クンバカ）） — シータリー（śītalī）の実践法エントリ"
tags: [yoga, pranayama, kumbhaka, hatha, hyp]
timestamp: 2026-06-14
practice_type: プラーナーヤーマ（クンバカ）
sanskrit: śītalī
source: 05_Sources/HYP_Svatmarama_fulltext.md
hyp_chapter: 2
hyp_section: Sitali
---

# シータリー（śītalī）

## 区分
**区分A — HYP確認済み**（HYP 第2章「Sitali」）

## 読み
シータリー

## 語源
śītala（シータラ / 冷たい・涼しい）＋ -ī（女性名詞語尾）。
直訳は「冷却するもの」。

## 原典記述（HYP 第2章）

> As in the above (Sitkari), the tongue to be protruded a little out of the lips, when the air is drawn in. It is kept confined, as before, and then expelled slowly through the nostrils.

> This Sitali Kumbhaka cures colic, (enlarged) spleen, fever, disorders of bile, hunger, thirst, and counteracts poisons.

*出典: 05_Sources/HYP_Svatmarama_fulltext.md — Chapter 2, Sitali*

## 解説
八種のクンバカの第四。舌を唇の外へ少し突き出して吸気し、クンバカの後に鼻孔からゆっくり呼気する冷却呼吸法。
疝痛・脾臓肥大・発熱・胆液の障害・飢え・渇きを治し、毒を打ち消すとされる。
[Sītkārī](sitkari.md)の類似技法。

## 関連
- [pranayama](../01_Vocabulary/pranayama.md)
- [sitkari](sitkari.md)
- [bhastrika](bhastrika.md)
